"""Swiss Life Hall Hannover source.

Fetches upcoming concerts and events from Swiss Life Hall website.
Uses HC-Kartenleger card system selectors for HTML parsing.
"""
...
@register_source("swiss_life_hall")
class SwissLifeHallSource(BaseSource):
    """Scraper for Swiss Life Hall Hannover.

    Fetches upcoming concerts and events from the venue website.
    Uses HC-Kartenleger card system selectors.

    Website: https://www.swisslife-hall.de/events/

    Attributes:
        source_name: "Swiss Life Hall"
        source_type: "concert"
    """

    source_name: ClassVar[str] = "Swiss Life Hall"
    source_type: ClassVar[str] = "concert"
    max_events: ClassVar[int | None] = 15

    # Configuration
    URL: ClassVar[str] = "https://www.swisslife-hall.de/events/"
    BASE_URL: ClassVar[str] = "https://www.swisslife-hall.de"
    ADDRESS: ClassVar[str] = "Ferdinand-Wilhelm-Fricke-Weg 8, 30169 Hannover"
    ...
    def fetch(self) -> list[Event]:
        """Fetch concert events from Swiss Life Hall.
    ...
    def _extract_subtitle(self, item: Tag, title: str) -> str:
        ...
        # Avoid returning the title as subtitle
